...
def main():
    ...
    with open("fullfinal.mp4", "rb") as video_file:
        video_bytes = video_file.read()

    # Convert to base64
    base64_video = base64.b64encode(video_bytes).decode()

        # HTML content (description + video side by side)
    video_html = f"""
    <div class="video-content">
        ...
        <div class="description-div">
            <div class="info-text">
                <h2 class="info-icon"> AutoMARK </h2>
                <span> Designed to transform traditional assessment methods with </span>
                <span class="info-core">AI-driven automation</span>. By intelligently analyzing 
                <span class="info-key">multiple-choice responses</span>, it ensures 
                <span class="info-focus">precision, consistency, and fairness</span> at scale—freeing educators 
                from repetitive grading tasks. With a focus on <span class="info-brand">Speed</span> and 
                <span class="info-brand">Accuracy</span>. Beyond saving educators from the burden of repetitive grading, 
                <span class="info-key" style="font-weight: 700;">AutoMARK</span> empowers to increase 
                <span class="info-focus">efficiency,</span> maintain high standards, and focus more on 
                <span class="info-core">Teaching</span> and 
                <span class="info-core">Mentoring.</span>
            </div>
        </div>
    </div>
    """
    ...
    st.markdown("---")
    st.markdown("""
       <h2><strong style="color: #667eea;"><i class="fa-solid fa-code-branch"></i></strong> <strong>Features</strong></h2>
     """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            if st.button("➲  Start AI Grading", type="primary", use_container_width=True):
                ...
                if error:
                    ...
                    st.info("📄 This document does not appear to be an MCQ answer sheet.")
                    st.markdown(f"""
                    ...
                    if ("mcqs" in result and isinstance(result["mcqs"], list) and len(result["mcqs"]) > 0): 
                        valid_mcqs = [
                            mcq for mcq in result["mcqs"] 
                            if mcq.get("available_options") 
                            and len(str(mcq.get("available_options"))) >= 3  # At least A, B, C
                            and mcq.get("correct_answer") 
                            and mcq.get("student_answer")
                        ]
                        ...
                        st.info("📄 This document does not appear to be an MCQ answer sheet.")
                        st.markdown("""
                            <div style="background: #fee2e2; color: black; border: 1px solid #fecaca; padding: 1.5rem; 
                                        border-radius: 10px; margin: 1rem 0 2rem;">
                                <h4 style="color: #dc2626; margin-top: 0;">Possible Causes:</h4>
                                <ul>
                                    <li>Image does not contain MCQs</li>
                                    <li>MCQs are too faint/blurry</li>
                                    <li>Answer markings were not detected properly</li>
                                </ul>
                            </div>
                            """, unsafe_allow_html=True)
                        st.markdown('<h4><i class="fa-solid fa-camera"></i> Image Quality Tips</h4>', unsafe_allow_html=True)
    ...
    if uploaded_file is not None and result is not None and "mcqs" in result and len(result["mcqs"]) > 0:  
        display_results(result, processing_time) #if len(valid_mcqs) > 0:
    ...
    st.markdown(
        ...
        <div class="note-box">
            <div class="note-text">
                ...
                <span class="note-brand">AutoMARK</span> is currently developing 
                <span class="note-agentic">advanced Agentic AI tools</span>, including 
                <span class="note-feature">Assessment Builders</span> and 
                <span class="note-highlight">Google Classroom</span> integration.
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    # FAQ Section
    st.markdown("""
       <h2><i style="padding-right: 5px;" class="fa-solid fa-comments"></i><strong style="color: #667eea;">FAQs</strong></h2>
     """, unsafe_allow_html=True)
